...
if ( ! function_exists( __NAMESPACE__ . '\\display_single_post_mentioned_in_article' ) ) {
     ...
     * pulling an Elementor loop-item template with its CSS inline so styling shows immediately.
     *
     * Shortcode attributes:
     *   must_have_thumbnail (bool) – if true, only include profiles that have a featured image. Default false.
     *
     * @param array $atts {
     *   @type bool $must_have_thumbnail
     * }
     * @return string HTML (or a hidden style if no profiles).
     */
    function display_single_post_mentioned_in_article( $atts = [] ) {
        // parse attributes, defaulting must_have_thumbnail to false
        $atts = shortcode_atts( [
        ...
        if ( ! is_single() ) {
            return 'This shortcode is only for single post pages.';
                ...
                // pull the template ID from your ACF “display_single_profile_press_releases” option
$template_id = get_field( 'display_single_post_mentioned_in_article', 'option' );
if (!$template_id ) return;
        ...
        if ( have_rows( 'profiles', $post->ID ) ) {
            while ( have_rows( 'profiles', $post->ID ) ) {
                ...
                // if must_have_thumbnail is true, enforce it
        ...
        // Preserve original post context
        $original_post = $post;
        //$template_id   = 43738;
        ...
        .display_single_post_mentioned_in_article{width:100%;display:block}
            .display_single_post_mentioned_in_article .shortcode{width:100%;display:grid;grid-template-columns:repeat(6,1fr);gap:1rem;}
            @media (max-width:600px){
                .display_single_post_mentioned_in_article .shortcode{grid-template-columns:repeat(2,1fr)!important;}
            }
        </style>';

        echo '<div class="display_single_post_mentioned_in_article"><div class="shortcode">';
        ...
        foreach ( $profile_ids as $pid ) {
            ...
            // swap global $post so Elementor dynamic tags work
            $GLOBALS['post'] = $profile_post;
            setup_postdata( $profile_post );

            // render your loop-item template
