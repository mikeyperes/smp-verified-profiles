        ...
        <div class="hpc-ui hpc-primary-entity" data-hpc-primary-entity data-ajax-url="<?php echo esc_url( admin_url( 'admin-ajax.php' ) ); ?>" data-action="<?php echo esc_attr( (string) $manager->config( 'ajax_action', 'hexa_save_primary_entity' ) ); ?>" data-nonce-field="<?php echo esc_attr( (string) $manager->config( 'nonce_field', 'nonce' ) ); ?>" data-nonce="<?php echo esc_attr( $nonce ); ?>" data-type-map="<?php echo esc_attr( wp_json_encode( $type_map ) ); ?>">
            <section class="hpc-card hpc-primary-settings">
                ...
                <div class="hpc-primary-entity-fields">
                    <label class="hpc-field"><span>Website type</span><select class="hpc-primary-site-type"><?php foreach ( $manager->site_types() as $value => $label ) : ?><option value="<?php echo esc_attr( $value ); ?>" <?php selected( $site_type, $value ); ?>><?php echo esc_html( $label ); ?></option><?php endforeach; ?></select><small>Determines the read-only semantic entity type.</small></label>
...
function updateDerivedType(root){var select=root.querySelector('.hpc-primary-site-type');var output=root.querySelector('.hpc-primary-derived-type');if(!select||!output)return;var map={};try{map=JSON.parse(root.dataset.typeMap||'{}');}catch(e){}var item=map[select.value]||{value:'auto',label:'Automatic'};output.value=item.label||'Automatic';output.textContent=item.label||'Automatic';output.dataset.value=item.value||'auto';}
