        ...
        if (!function_exists(__NAMESPACE__ . '\\create_unclaimed_profiles_user')) {
            ...
            if (!function_exists(__NAMESPACE__ . '\\create_wordpress_user')) {
                ...
                if (!function_exists(__NAMESPACE__ . '\\check_user_unclaimed_exists')) {
                    function check_user_unclaimed_exists($slug = 'unclaimed-profiles') {
                        ...
                        if ($user_exists) {
                            ...
                            $user_info = [
                                ...
                                'number_of_posts' => count_user_posts($user->ID, 'profile'), // Count of posts of type 'profile'
...
";
                    ...
                    if (!function_exists(__NAMESPACE__ . '\\check_profile_taxonomies')) {
                        function check_profile_taxonomies() {
                            write_log("Checking profile taxonomies", false);
                    
                            // Check if categories are assigned to 'profile' post type
                            $categories_assigned = (get_object_taxonomies('profile') && taxonomy_exists('category')) ? true : false;
                            
                            // Check if tags are assigned to 'profile' post type
                            $tags_assigned = (get_object_taxonomies('profile') && taxonomy_exists('post_tag')) ? true : false;
...
";
                        ...
                        if (!function_exists(__NAMESPACE__ . '\\fix_profile_taxonomies')) {
                            function fix_profile_taxonomies() {
                                ...
                                // Assign categories to 'profile' post type if missing
                                if (!taxonomy_exists('category')) {
                                    register_taxonomy_for_object_type('category', 'profile');
                                    write_log("Categories assigned to 'profile' post type", false);
                                }
                        
                                // Assign tags to 'profile' post type if missing
                                if (!taxonomy_exists('post_tag')) {
                                    register_taxonomy_for_object_type('post_tag', 'profile');
                                    write_log("Tags assigned to 'profile' post type", false);
                                }
...
/* The following commented-out code is related to WP CLI functionality and remains unchanged:

if (defined('WP_CLI') && WP_CLI) {
    class Schema_Markup_Update_Command {

        public function update($args, $assoc_args) {
            $batch = (int) $assoc_args['batch'];
            $query_args = [
                'post_type' => 'profile',
                'posts_per_page' => $batch,
                'fields' => 'ids',
            ...
            if ($query->have_posts()) {
                foreach ($query->posts as $post_id) {
                    ...
                    WP_CLI::log("Updated schema markup for post ID: {$post_id}");
                }
            } else {
                WP_CLI::log("No more posts to update.");
            }
        }
    }

    WP_CLI::add_command('schema_markup', 'Schema_Markup_Update_Command');
}

https://yourdomain.com/wp-admin/admin.php?update_schema=1

https://yourdomain.com/wp-admin/admin.php?update_schema=1&batch=10

https://herforward.com/wp-admin/admin.php?update_schema=1&batch=500
